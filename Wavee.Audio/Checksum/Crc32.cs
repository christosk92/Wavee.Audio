...
namespace Wavee.Audio.Checksum;

/// <summary>
/// <see cref="Crc32"/> implements the CRC-32 checksum algorithm using the standard polynomial.
///
...
/// * XorOut = false
/// </summary>
public sealed class Crc32 : IMonitor
{
    private uint state;

    // Assuming CRC32 is a precomputed lookup table
    ...
    // Processes a byte and updates the CRC state.
    public void ProcessByte(byte value)
    {
        state = (state << 8) ^ CRC32[0][((state >> 24) ^ value) & 0xFF];
    }

    // Processes a byte buffer and updates the CRC state.
    public void ProcessBufBytes(ReadOnlySpan<byte> buf)
    {
        int index = 0;

        while (index + 8 <= buf.Length)
        {
            uint word = (uint)(buf[index] << 24) | (uint)(buf[index + 1] << 16) | (uint)(buf[index + 2] << 8) |
                        buf[index + 3];
            uint xor = state ^ word;

            state = CRC32[7][(xor >> 24) & 0xFF] ^
                    CRC32[6][(xor >> 16) & 0xFF] ^
                    CRC32[5][(xor >> 8) & 0xFF] ^
                    CRC32[4][(xor) & 0xFF] ^
                    CRC32[3][buf[index + 4]] ^
